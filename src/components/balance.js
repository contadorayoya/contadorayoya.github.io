...
const fetchBalanceData = async () => {
  ...
  try {
    ...
    const docId = `${selectedEmpresa}_${selectedAño}`.replace(/\s+/g, '_');
    
    // Obtener el documento directamente usando el ID construido
    const totalesDocRef = doc(db, 'totales', docId);
    const totalesSnapshot = await getDoc(totalesDocRef);
    
    if (!totalesSnapshot.exists()) {
      setError(`No se encontraron datos para ${selectedEmpresa} en el año ${selectedAño}`);
      setLoading(false);
      return;
    }
    
    const totalesData = totalesSnapshot.data();
    ...
    // Preparar el array de balance
    const balanceArray = [];
    
    // Campos que sabemos que no son tipos
    const camposMetadata = ['empresa', 'año', 'fechaActualizacion'];
    
    // Procesar todos los tipos encontrados en el documento
    Object.entries(totalesData).forEach(([key, value]) => {
      // Ignorar campos de metadata
      if (camposMetadata.includes(key)) {
        return;
      }
      
      // Si el valor es un objeto con debito/credito
      if (value && typeof value === 'object' && ('debito' in value || 'credito' in value)) {
        const debe = parseFloat(value.debito || 0);
        const haber = parseFloat(value.credito || 0);
    ...
    try {
      const balanceConfigRef = doc(db, 'balances', balanceDocId);
